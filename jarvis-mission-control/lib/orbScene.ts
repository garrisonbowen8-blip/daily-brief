...
void main() {
  ...
  if (d > 0.5) discard;
  // Sharper falloff: tight core, hard edge
  float a = smoothstep(0.5, 0.18, d);
  // Brighter whites on hot sparks, vivid color on rest
  vec3 col = mix(uColor * 0.6, mix(uColor, vec3(1.0), 0.95), pow(vBright, 2.0));
  gl_FragColor = vec4(col * (0.5 + vBright * 0.5), a * vBright * 0.85);
}
`;
...
type CloudSpec = { count: number; patches: number; rMin: number; rMax: number; patchSpread: number; baseSize: number };
...
function buildCloud(spec: CloudSpec, color: THREE.Color) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (scatter) {
      ...
      p.copy(c.dir).multiplyScalar(c.r + (Math.random() - 0.5) * 0.05)
        .addScaledVector(c.u, gx + jitter).addScaledVector(c.v, gy + jitter);
    }
    pos[i * 3] = p.x; pos[i * 3 + 1] = p.y; pos[i * 3 + 2] = p.z;
    const hot = Math.random();
    size[i] = baseSize * (0.5 + Math.random());
    bright[i] = hot < 0.06 ? 1.0 : 0.25 + Math.random() * 0.5;
  ...
  const uniforms = { uTime: { value: 0 }, uLevel: { value: 0 }, uColor: { value: color } };
...
export function initOrb(canvas: HTMLCanvasElement, size = 340): () => void {
  ...
  // Fewer, larger, brighter particles = crisp defined look instead of fuzzy cloud
  const outer = buildCloud({ count: 10000, patches: 80, rMin: 1.8, rMax: 2.2, patchSpread: 0.28, baseSize: 3.8 }, color);
  scene.add(outer.points);

  const inner = buildCloud({ count: 3500, patches: 35, rMin: 0.75, rMax: 1.2, patchSpread: 0.22, baseSize: 3.2 }, color);
  scene.add(inner.points);

  const traceA = buildTraces(140, 1.85, 2.15, color);
  const traceB = buildTraces(55, 0.8, 1.15, color);
  ...
  for (let i = 0; i < 4; i++) {
    const mat = new THREE.MeshBasicMaterial({
      color: color.clone(), transparent: true, opacity: 0.18 + Math.random() * 0.15,
      blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide,
    });
    const arc = new THREE.Mesh(
      new THREE.TorusGeometry(2.05 + i * 0.22, 0.008, 6, 180, Math.PI * (0.5 + Math.random())), mat
    );
  ...
  const glowMat = new THREE.SpriteMaterial({
    map: new THREE.CanvasTexture(glowCanvas), color: color.clone(),
  ...
  const frame = () => {
    ...
    const synth =
      s === "speaking" ? 0.3 + 0.15 * Math.sin(t * 9) + 0.08 * Math.sin(t * 23)
      : s === "listening" ? 0.14 + 0.05 * Math.sin(t * 6)
      : s === "thinking" ? 0.22 + 0.07 * Math.sin(t * 3.5)
  ...
  return () => {
    ...
    traceA.geo.dispose(); traceA.mat.dispose();
    traceB.geo.dispose(); traceB.mat.dispose();
    for (const a of arcs) { a.mesh.geometry.dispose(); a.mat.dispose(); }
    glowMat.dispose();
  };
}
